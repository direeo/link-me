...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // Rate limiting check - combine IP and email for stricter limiting
        ...
        if (!rateLimit.allowed) {
            return NextResponse.json(
                {
                    success: false,
                    message: `Too many login attempts. Please try again in ${Math.ceil((rateLimit.retryAfter || 1800) / 60)} minutes.`,
                },
                { status: 429 }
            );
        }
